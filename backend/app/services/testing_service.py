...
def run_testing(app, test_id):
    with app.app_context():
        ...
        try:
            ...
            # Kolom label bisa 'label' atau 'emotion' (tergantung training)
            if 'label' in df.columns:
                y_true = df['label'].tolist()
            else:
                y_true = df['emotion'].tolist()

            # Muat model artifacts
            artifacts = joblib.load(training.model_path)

            # Prediksi
            if training.config.algorithm == 'Lexicon-NB':
                y_pred = predict_lexicon(texts, artifacts)
            elif training.config.algorithm == 'IndoBERT-KNN':
                y_pred = predict_indobert(texts, artifacts)
            else:
                raise ValueError("Algoritma tidak didukung")
